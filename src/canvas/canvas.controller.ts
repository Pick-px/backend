...
export class CanvasController {
  ...
  @ApiTags('canvas/pixels')
  @ApiOperation({ summary: '특정 캔버스의 모든 픽셀 데이터 조회', description: 'canvas_id로 해당 캔버스의 전체 픽셀 데이터를 압축하여 반환합니다.' })
  @ApiQuery({ name: 'canvas_id', required: true, description: '조회할 캔버스 ID' })
  @ApiResponse({
    status: 200,
  ...
  async getAllPixels(@Query('canvas_id') canvas_id?: string) {
    ...
    return {
      ...
      data: {
        ...
        canvasSize: {
          width: canvas?.sizeX ?? 0,
          height: canvas?.sizeY ?? 0
        }
      }
    };
  }
