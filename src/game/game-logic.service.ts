...
export class GameLogicService {
  ...
  async handleSendResult(
    ...
    if (canvasInfo?.metaData?.endedAt && now > canvasInfo.metaData.endedAt) {
      console.warn(`[handleSendResult] 종료된 캔버스에 색칠 시도 차단: userId=${userId}, canvas_id=${data.canvas_id}`);
    ...
    if (data.result) {
      ...
      const canvasInfoAfter = await this.canvasService.getCanvasById(data.canvas_id);
      const nowAfter = new Date();
      if (canvasInfoAfter?.metaData?.endedAt && nowAfter > canvasInfoAfter.metaData.endedAt) {
        console.warn(`[handleSendResult] 정답 처리 후 종료 시간 만료 감지, forceGameEnd 호출: canvas_id=${data.canvas_id}`);
        await this.forceGameEnd(data.canvas_id, server);
      }
      return;
    } else {
      ...
      if (life <= 0) {
        ...
        const canvasInfo = await this.canvasService.getCanvasById(data.canvas_id);
        const now = new Date();
        const isGameTimeOver = canvasInfo?.metaData?.endedAt && now > canvasInfo.metaData.endedAt;
        console.log(`[GameLogicService] 게임 종료 조건 체크: canvasId=${data.canvas_id}, 생존자=${aliveUserIds.length}명, 게임시간종료=${isGameTimeOver}`);
        // 남은 생존자가 없거나 게임 시간이 지났으면 게임 종료
        if (aliveUserIds.length === 0 || isGameTimeOver) {
          console.log(`[GameLogicService] 게임 종료 조건 만족: 생존자=${aliveUserIds.length}명, 시간종료=${isGameTimeOver}`);
          // 랭킹 계산 시간 측정 시작
          ...
          console.log(`[GameLogicService] 랭킹 계산 및 결과 브로드캐스트 준비까지 소요: ${rankingEnd - rankingStart}ms`);
          ...
          try {
            ...
            console.log(`[GameLogicService] 워커 큐에 canvas-history 잡 추가 완료: canvasId=${data.canvas_id}`);
          } catch (e) {
            console.error(`[GameLogicService] 워커 큐에 canvas-history 잡 추가 실패: canvasId=${data.canvas_id}`, e);
          }
          server.to(`canvas_${data.canvas_id}`).emit('game_result', { results: ranked });
          console.log(`[GameLogicService] 게임 결과 브로드캐스트 완료: canvasId=${data.canvas_id}`);
        }
        // ---
      }
      return;
  ...
  private async insertUserToGameResult(canvasId: string, userId: string) {
    try {
      ...
      if (!color) {
        ...
        console.log(`[GameLogicService] 기본 색 생성 및 저장: userId=${userId}, color=${color}`);
      }
      ...
      console.log(`[GameLogicService] game_user_result 삽입 완료: userId=${userId}, username=${username}, color=${color}`);
    } catch (error) {
      console.error(`[GameLogicService] game_user_result 삽입 실패: userId=${userId}, canvasId=${canvasId}, 에러:`, error);
    }
  }
  ...
  private async createCanvasHistory(canvasId: string) {
    try {
      console.log(`[GameLogicService] canvas_history 생성 시작: canvasId=${canvasId}`);
      ...
      if (existingHistory.length > 0) {
        console.log(`[GameLogicService] canvas_history 이미 존재: canvasId=${canvasId}`);
        return;
      }
      ...
      console.log(`[GameLogicService] canvas_history 생성 완료: canvasId=${canvasId}`);
    } catch (error) {
      console.error(`[GameLogicService] canvas_history 생성 실패: canvasId=${canvasId}`, error);
    }
  }
  ...
  async forceGameEnd(canvasId: string, server?: any) {
    try {
      ...
      // 랭킹 산정
      const ranked = this.calculateRanking(userStats);
      console.log(`[GameLogicService] forceGameEnd 랭킹 계산 완료:`, ranked.map(r => `${r.username}(rank=${r.rank}, own=${r.own_count}, try=${r.try_count}, dead=${r.dead})`));
      // 랭킹 계산 시간 측정 끝
      const rankingEnd = Date.now();
      console.log(`[GameLogicService] forceGameEnd 랭킹 계산 및 결과 브로드캐스트 준비까지 소요: ${rankingEnd - rankingStart}ms`);
      // 게임 결과를 DB에 저장 (rank만 업데이트)
      ...
      try {
        ...
        console.log(`[GameLogicService] 워커 큐에 canvas-history 잡 추가 완료: canvasId=${canvasId}`);
      } catch (e) {
        console.error(`[GameLogicService] 워커 큐에 canvas-history 잡 추가 실패: canvasId=${canvasId}`, e);
      }
      // 결과 브로드캐스트
      if (server) {
        server.to(`canvas_${canvasId}`).emit('game_result', { results: ranked });
        console.log(`[GameLogicService] forceGameEnd 결과 브로드캐스트 완료: canvasId=${canvasId}`);
      }
    } catch (err) {
