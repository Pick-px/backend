...
export class GameStateService {
  ...
  async getUserLife(canvasId: string, userId: string): Promise<number> {
    const life = parseInt(
      (await this.redis.hget(`game:${canvasId}:user:${userId}`, 'life')) || '0',
      10
    );
  ...
  async getUserDead(canvasId: string, userId: string): Promise<boolean> {
    const dead =
      (await this.redis.hget(`game:${canvasId}:user:${userId}`, 'dead')) ===
      '1';
    return dead;
  }
  ...
  async getUserTryCount(canvasId: string, userId: string): Promise<number> {
    const tryCount = parseInt(
      (await this.redis.hget(`game:${canvasId}:user:${userId}`, 'try_count')) ||
        '0',
      10
    );
  ...
  async getUserOwnCount(canvasId: string, userId: string): Promise<number> {
    const ownCount = parseInt(
      (await this.redis.hget(`game:${canvasId}:user:${userId}`, 'own_count')) ||
        '0',
      10
    );
  ...
  async getUserColor(canvasId: string, userId: string): Promise<string> {
    const color =
      (await this.redis.hget(`game:${canvasId}:user:${userId}`, 'color')) || '';
    return color;
  }
